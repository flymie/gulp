...
(function(window,underfined){
	...
	xwPop.autoHide = autoHide;  //新增的自动消失
	function msg(o){
	...
	function modal(o){
		...
		if(_o.mask){
			modalBox.setAttribute("class",'xw-pop-modalbox fixMid mask');
		}else{
			modalBox.setAttribute("class",'xw-pop-modalbox fixMid');
		}

		//弹窗位置，位置决定了所在的动画。默认在中间位置，现在修改到底部
		if(_o.location.trim() == 'btm'){
			modalBox.className = modalBox.className.replace(/fixMid/,'fixBtm');
		}
		modalBody[modalBody.length] = "<div class=\"xw-pop-modal-section\" style=\"width:"+_o.width+"\">";
		if(!_o.hasClose){
			modalBody[modalBody.length] = "<div class=\"xw-pop-modal-header\">"+_o.title+"</div>";
		}else{
		...
		if(_o.type == 'confirm'){
			modalBody[modalBody.length] = "<div class=\"xw-pop-modal-btn xw-confirm\">";
	...
	//新增的自动消失弹窗
	function autoHide(o){
		var modalBox = document.createElement("div"),modalBody=[],_o = $.extend({},xwPop.autoHideInit,o);
		if(_o.mask){
			modalBox.setAttribute("class",'xw-pop-modalbox mask');
		}else{
			modalBox.setAttribute("class",'xw-pop-modalbox');
		}
		modalBody[modalBody.length] = "<div class=\"xw-pop-modal-section xw-autoHide\" style=\"width:"+_o.width+"\">"+_o.text+"</div>";
		modalBox.innerHTML = modalBody.join("");
		$("body").append($(modalBox));
	...
	var xw = window['xw'] = { //为了简化使用对象
		...
		alert:function(txt,fn){
			xwPop.modal({
				title:'',
				text:"<p style='padding-bottom:1rem;text-align:center;'>"+txt+"<p>",
				location:'mid',
				callBack:fn,
				hasClose:false,  //默认有关闭按钮
			})
		},
